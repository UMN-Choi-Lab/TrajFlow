...
class FloMo(nn.Module):
    ...
    def __init__(self, hist_size, pred_steps, alpha, beta, gamma, num_feat, num_in=2, B=15., momentum=0.2,
    ...
    def _encode_conditionals(self, x, feat=None):
        ...
        if feat is not None:
            x_in = torch.cat([x_in, feat], dim=-1)
    ...
    def _inverse(self, y_true, x, feat=None):
        if self.norm_rotation:
            x, y_true, angle = self._normalize_rotation(x, y_true)

        x_t = x[..., -1:, :]
        x_enc = self._encode_conditionals(x, feat)  # history encoding
    ...
    def sample(self, n, x, feat=None):
        with torch.no_grad():
            ...
            return samples_abs[0], log_probs

    def log_prob(self, y_true, x, feat=None):
        z, log_abs_jacobian_det = self._inverse(y_true, x, feat)
        normal = Normal(0, 1, validate_args=True)
        return normal.log_prob(z).sum(1) + log_abs_jacobian_det