...
# 		self._times = times
# 		self._a, self._b, self._two_c, self._three_d = self._natural_cubic_spline_coeffs(times, path)
...
# 	def _natural_cubic_spline_coeffs_without_missing_values(self, times, path):
...
# 			system_rhs[..., -1] = 0
# 			system_rhs[..., 1:] += path_diffs_scaled
# 			knot_derivatives = self._tridiagonal_solve(system_rhs, time_diffs_reciprocal, system_diagonal, time_diffs_reciprocal)
...
# 	def _natural_cubic_spline_coeffs_with_missing_values(self, t, path):
# 		if len(path.shape) == 1:
...
# 			return self._natural_cubic_spline_coeffs_with_missing_values_scalar(t, path)
# 		else:
# 			a_pieces = []
# 			b_pieces = []
# 			two_c_pieces = []
...
# 				a, b, two_c, three_d = self._natural_cubic_spline_coeffs_with_missing_values(t, p)
# 				a_pieces.append(a)
# 				b_pieces.append(b)
...
# 	def _natural_cubic_spline_coeffs_with_missing_values_scalar(self, times, path):
...
# 	 	b_pieces_no_nan,
# 	 	two_c_pieces_no_nan,
# 	 	three_d_pieces_no_nan) = self._natural_cubic_spline_coeffs_without_missing_values(times_no_nan, path_no_nan)
...
# 	# time stamps.)
# 	def _natural_cubic_spline_coeffs(self, t, X):
...
# 			a, b, two_c, three_d = self._natural_cubic_spline_coeffs_with_missing_values(t, X.transpose(-1, -2))
# 		else:
# 			# Can do things more quickly in this case.
# 			a, b, two_c, three_d = self._natural_cubic_spline_coeffs_without_missing_values(t, X.transpose(-1, -2))
