...
class EthUcyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = history.values
        y = future[['x_pos', 'y_pos']].values
        
        # Pad with zeros if necessary
        if x.shape[0] < self.history_frames:
            pad_width = ((self.history_frames - x.shape[0], 0), (0, 0))
            x = np.pad(x, pad_width, mode='constant')
        
        if y.shape[0] < self.future_frames:
            pad_width = ((0, self.future_frames - y.shape[0]), (0, 0))
...
class EthUcy():
    ...
    def create_data_loader(self, batch_size=32, shuffle=True):
        dataset = EthUcyDataset(self.scenes)
...
for i, (batch_x, batch_y) in enumerate(data_loader):
    ...
    print(f"Target shape (batch_y): {batch_y.shape}")
